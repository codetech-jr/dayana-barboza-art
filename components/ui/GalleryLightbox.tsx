 ...
 *
 * - Uses React Portal to escape z-index constraints of page layouts (e.g., fixed Nav on top).
 * - Massive high-res center image perfectly contained with no height collapse.
 * - Full keyboard navigation: Left/Right arrows + Escape to close.
 * - Direct CRO Embed: High-converting WhatsApp CTA dynamically referencing piece name.
 */
export function GalleryLightbox({
  ...
  const [activePiece, setActivePiece] = useState<GalleryPiece | null>(null);
  ...
  // Sync state so AnimatePresence has content to render during its exit animation
  useEffect(() => {
    if (isOpen && selectedIndex !== null) {
      // eslint-disable-next-line react-hooks/set-state-in-effect
      setActivePiece(pieces[selectedIndex]);
      setActiveIndex(selectedIndex);
    }
  }, [isOpen, selectedIndex, pieces]);
  ...
  // Prevent hydration mismatch and ensure document.body is available for createPortal
  ...
  const whatsappUrl = activePiece
    ? buildWhatsAppUrl('lightboxQuote', locale, activePiece.title[locale])
    : '';
    ...
    <AnimatePresence>
      {isOpen && activePiece && (
        <motion.div
          ...
          role="dialog"
          aria-modal="true"
          aria-label={activePiece.title[locale]}
        >
          ...
          <div className="shrink-0 flex items-center justify-between w-full max-w-7xl mx-auto z-10 h-11">
            <div className="flex items-center gap-3">
              ...
              <span className="font-body text-xs uppercase tracking-[0.16em] text-dba-accent font-medium">
                {dict.gallery.filters[activePiece.category]}
              </span>
            </div>
          ...
          <div className="relative flex-1 min-h-0 w-full max-w-6xl mx-auto my-3">
            {/* Prev button */}
            <button
              type="button"
              onClick={handlePrev}
            ...
            <div className="absolute inset-0 w-full h-full">
              <Image
                ...
                alt={`${activePiece.title[locale]} — ${dict.gallery.filters[activePiece.category]}`}
                fill
                priority
